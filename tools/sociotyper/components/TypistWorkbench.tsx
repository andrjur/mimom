...
const BYOK_DEFAULTS: Record<Exclude<ByokSettings['provider'], 'custom'>, Pick<ByokSettings, 'baseUrl' | 'model'>> = {
  just: { baseUrl: 'https://api.justwoker.icu/v1', model: 'gpt-5.6-luna' },
...
const makeConnection = (settings: ByokSettings, checkedModel?: string): ApiConnection => ({
  ...
  label: settings.provider === 'knyazev' ? 'Knyazev AI / Gonka' : settings.provider === 'just' ? 'JustDoWork' : settings.provider === 'custom' ? 'Другой API' : settings.provider,
