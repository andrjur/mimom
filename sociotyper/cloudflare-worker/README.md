...
6. В логах отфильтруйте поле `traceId` по этому значению. Будут видны события `typist.analysis.start`, четыре `typist.model.request/response` и `typist.analysis.complete`.
